...
from util.file_explorer import NEG_DATA_DIR, POS_DATA_DIR, MODELS_DIR, CONFIG_FILENAME, DATA_PRED_DIR, SEP, makedir
...
def predict(model_dir: str) -> None:
    ...
    for set_filepath in TST_SETS_FILEPATHS:
        ...
        y_pred = []
        for j, model_filepath in enumerate(glob.glob(os.path.join(model_dir, MODELS_DIR, "*"))):
            # Load model
            ...
            save_model_prediction(model_name, fs, y_pred[j])

        # Save comb results
        if j > 0:
            y_pred = np.mean(y_pred, axis=0)
            model_name = config["model_name"] + "comb" + "".join(str(i) for i in range(1, j + 2))
            save_model_prediction(model_name, fs, y_pred)

def save_model_prediction(model_name: str, fs: FragmentedSet, fragments_prediction: np.ndarray) -> None:
...
def to_sequence_prediction(fs: FragmentedSet, fragments_prediction: np.ndarray) -> tuple[list[float], list[str]]:
    pred = []
    frags = []
